app.controller("accountDevCtrl", function(
  $scope,
  $stateParams,
  accountDevSrvc
) {
  ...
  $scope.submitComment = comment => {
    if (comment) {
      accountDevSrvc
        .submitComment($stateParams.id, comment, $scope.curUser.id)
        .then(comments => {
          $scope.userComments = comments.data;
        });
    } else {
  ...
  $scope.getCommentsById = () => {
    accountDevSrvc.getCommentsById($stateParams.id).then(comments => {
  ...
  $scope.getProjectsById = () => {
    accountDevSrvc.getProjectsById($stateParams.id).then(projects => {
  ...
  $scope.getUserSkills = id => {
    accountDevSrvc.getUserSkills(id).then(skills => {
  ...
  $scope.getUpdatedUser = userid => {
    accountDevSrvc.getUserById(userid).then(user => {
  ...
  $scope.getLoggedInUser = () => {
    accountDevSrvc.getLoggedInUser().then(user => {
  ...
  $scope.getUserByParams = () => {
    ...
    if ($scope.curUser.id != param) {
      accountDevSrvc.getUserByParams(param).then(user => {
  ...
  $scope.subChanges = (newFirstName, newLastName, newBio, newSkills) => {
    ...
    accountDevSrvc.updateUser($stateParams.id, newUser).then(updatedUser => {
  ...
  $scope.updatePicture = avatar => {
    ...
    for (let i = 0; i < 17; i++) {
      if (avatar === "avatar" + i) {
        ...
        // console.log(imgUrl)
        accountDevSrvc.updateImg($scope.curUser.id, imgUrl).then(user => {
          $scope.curUser = Object.assign({}, user.data[0]);
          $scope.showPictures = false;
        });
      }
