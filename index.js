...
passport.use(new Auth0Strategy({
    ...
    db.getUserByAuthId([profile.id])
        .then(
            function(user) {
                if (user.length < 1) {
                    db.postUser(profile.id, profile.emails[0].value, profile.nickname, "I am a blank bio", "./img/avatars/1.svg", false).then(function(user) {
                        profile.user = user[0];
                        // console.log(user);
...
app.get('/auth/logout', function(req, res) {
    ...
    res.redirect('https://dylandoesprogramming.auth0.com/v2/logout?returnTo=' + 'http://www.devgive.net/#!/');
})
